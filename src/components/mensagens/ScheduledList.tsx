...
export default function ScheduledList({ statusFilter, emptyLabel }: Props) {
  ...
  const saveEdit = async () => {
    ...
    if (dt <= new Date()) return toast({ title: "A data deve ser futura", variant: "destructive" });
    setSaving(true);
        ...
        <CardContent>
          {messages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">{emptyLabel ?? "Nenhuma mensagem."}</p>
            </div>
          ) : (
            <MessagesList
              messages={messages}
